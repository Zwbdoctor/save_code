...
# when  keycode>254(unicode) set wScan = keycode and set wVk =
...
def KeyPressUper(keyCode, isShift):
    if isShift == True:
            ...
            if vk == -1:
                ...
                if win32api.GetKeyState(win32con.VK_CAPITAL) & 0x1 == 0x1:
                    if (c >= 'a' and c <= 'z') or (c >= 'A' and c <= 'Z'):
                        shift = not shift
                KeyPressUper(vk & 0xFF, shift)
        else:
            UniKeyPress(cC)
            # print cC


def KeyPressLower(KeyCode):
    KeyDown(VK_CODE_WIN[KeyCode])
    time.sleep(0.01)
    KeyUp(VK_CODE_WIN[KeyCode])

def KeyPressMore(KeyCodes=None):
    """组合键"""
    if not KeyCodes:
        return
    for k in KeyCodes:
        KeyDown(VK_CODE_WIN[k])
        time.sleep(0.3)
    for k in KeyCodes[::-1]:
        KeyUp(VK_CODE_WIN[k])
        time.sleep(0.3)
