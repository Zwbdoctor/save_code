...
from platform_crawler.configs.pwd import pkey
sec = pkey.get('wwyy')
from platform_crawler.apis.chaojiying import ChaojiyingClient
from platform_crawler.configs.excute_paths import ExecutePaths
# from apis.ShowapiRequest import ShowapiRequest
...
class Util:
    pag = pag
    cookiepath = ExecutePaths.ChromeCookiePath
    # 打码平台对象
    rc = ChaojiyingClient('cjyhhmt', 'cjyhhmt1234', '899827')
    # rc = ShowapiRequest(sec.get('un'), sec.get('pw'))
    ...
    def make_months(self, ys=None, ms=None, ye=None, mee=None, dur=1):
        mths = []
        year, month, cDay = time.strftime('%Y-%m-%d').split('-')
        ms = int(month) - dur if not ms else ms  # 开始月份 defaul: 前(dur)个月
    ...
    def make_dates(self, ys=None, ms=None, ye=None, me=None, dur=1):
        """
        生成一段时间中，每个月的开始和结束日期
        ...
        """
        kwargs = {'ys': ys, 'ms': ms, 'ye': ye, 'mee': me}
        if dur != 1:
            kwargs['dur'] = dur
        mths = self.make_months(**kwargs)
        year, month, cDay = time.strftime('%Y-%m-%d').split('-')
        z = lambda x: x if len(str(x)) == 2 else '0%s' % x
        dates = []
        for y, m in mths:
